    ...
    public String chat(@RequestBody String userMessage) throws IOException {
        ...
        if (lowerMsg.contains("donation")) {
            ...
            donations.forEach(d -> dbInfo.append(d.getName())
                    .append(" - ").append(d.getAmount()).append("\n"));
        }

        // --- Committee Contributions ---
        if (lowerMsg.contains("committee")) {
            List<Commity> contributions = form0Repo.findAll();
            int total = contributions.stream().mapToInt(Commity::getAmount).sum();
            dbInfo.append("Total committee contribution = ").append(total).append("\n");
        ...
        // --- Expenses ---
        if (lowerMsg.contains("expense")) {
            List<Expense> expenses = expenseRepo.findAll();
            double totalExpense = expenses.stream().mapToDouble(Expense::getExpense).sum();
            dbInfo.append("Total expenses = ").append(totalExpense).append("\n");
            dbInfo.append("Expense details:\n");
            expenses.forEach(e -> dbInfo.append(e.getPerson())
                    .append(" - ").append(e.getExpense()).append("\n"));
        }

        // --- Previous Year Amounts ---
        if (lowerMsg.contains("previous year")) {
            var prevAmounts = previousYearAmountDao.findAll();
            double totalPrev = prevAmounts.stream().mapToDouble(p -> p.getAmount()).sum();
            dbInfo.append("Total previous year amount = ").append(totalPrev).append("\n");
        }

        // --- Market Expenses ---
        if (lowerMsg.contains("market")) {
            List<Market> markets = marketRepository.findAll();
            double totalMarket = markets.stream().mapToDouble(Market::getExpense).sum();
            dbInfo.append("Total market expenses = ").append(totalMarket).append("\n");
            dbInfo.append("Market details:\n");
            markets.forEach(m -> dbInfo.append(m.getMaterial())
                    .append(" - ").append(m.getBuyer())
                    .append(" - ").append(m.getExpense()).append("\n"));
        }

        // --- Materials ---
        if (lowerMsg.contains("material")) {
            List<Material> materials = materialRepository.findAll();
            dbInfo.append("Materials list:\n");
            materials.forEach(m -> dbInfo.append(m.getName())
                    .append(" - ").append(m.getMaterial()).append("\n"));
        }

        // --- General Contributions ---
        if (lowerMsg.contains("contribution")) {
            List<Contribution> contributions = contributionRepository.findAll();
            double totalContribution = contributions.stream().mapToDouble(c -> c.getAmount()).sum();
            dbInfo.append("Total contributions = ").append(totalContribution).append("\n");
        ...
        // Send to Gemini (bot name handled inside GeminiService)
